...
- **自定义计划**：创建并管理自定义训练计划，支持自定义计划名、每日标题、休息日，以及可调整的训练日循环（增删/排序训练日）。之后修改计划（训练日名称、动作、训练日顺序）不会改写已保存的历史训练记录。
- **计划导入导出**：将选中的一个或多个计划导出为单个 JSON 文件；也可从 JSON 导入计划——允许重名，缺失的动作会自动创建。
- **两级动作标签**：按 身体部位 → 具体肌群 分类动作，支持多标签，可在设置中管理标签。
- **统计看板**：独立「统计」页展示 90 天 GitHub 风格活动热力图、部位训练频次（本周 / 本月 / 当前计划）、标识薄弱部位的月度平衡环、12 周趋势线与计划达成率。
- **交互式热力图**：在「统计」页查看带月份滚动和星期标签的 90 天训练一致性热力图。
- **渐进超负荷**：通过详细的训练历史与表现洞察跟踪你的长期进步。
- **体重记录**：记录体重并通过交互式折线图查看变化趋势。选择计划时展示该计划激活区间内的全部体重记录。
- **安全删除**：动作、计划、训练组、体重记录与训练会话均通过显式删除按钮 + 确认对话框删除，防止误删。
- **现代导航**：通过固定在页面底部的导航栏在 首页 / 记录 / 统计 / 我的 之间轻松切换。
- **Material Design 3**：全面遵循最新 MD3 规范、圆角统一的 UI 组件，Android 12+ 支持动态取色。
- **备份与导出**：完整应用备份（ZIP，WorkManager 自动定时）与选择性训练数据导出（JSON，支持日期范围过滤）。
- **多语言**：支持英文与简体中文。
- **字体大小可调**：在设置中调节 小 / 中 / 大，全局缩放应用排版，立即生效。
